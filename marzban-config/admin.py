...
{user_link} 🚀 欢迎使用 DC速连 管理面板！

👨‍💻 在此管理所有用户与节点代理。
📋 使用下方按钮快速操作。
🔍 也可通过 /user 命令查询或修改指定用户。
""".format(
...
def reset_usage_user_command(call: types.CallbackQuery):
    username = call.data.split(":")[1]
    bot.edit_message_text(
        f"⚠️ 确认操作？ This will Reset Usage of user `{username}`.",
        call.message.chat.id,
        call.message.message_id,
        parse_mode="markdown",
        reply_markup=BotKeyboard.confirm_action(
...
def add_time_step(message):
    ...
    msg = bot.send_message(
        message.chat.id,
        f"⚠️ 确认操作？ 此操作将调整所有用户的到期时间： <b>{days} Days</b>",
        parse_mode="html",
...
def delete_expired_confirm_command(call: types.CallbackQuery):
    bot.edit_message_text(
        f"⚠️ 确认操作？ This will *{call.data[16:].replace(':', ' ')} for All Users*‼️",
        call.message.chat.id,
        call.message.message_id,
        parse_mode="markdown",
...
➖➖➖➖➖➖➖➖➖
<b>操作人：</b> <a href="tg://user?id={message.chat.id}">{message.from_user.full_name}</a>"""
            try:
...
def revoke_sub_command(call: types.CallbackQuery):
    username = call.data.split(":")[1]
    bot.edit_message_text(
        f"⚠️ 确认操作？ 此操作将重置以下用户的订阅链接： `{username}`‼️",
        call.message.chat.id,
        call.message.message_id,
        parse_mode="markdown",
...
def links_command(call: types.CallbackQuery):
    ...
    with GetDB() as db:
        db_user = crud.get_user(db, username)
        if not db_user:
            return bot.answer_callback_query(call.id, "用户不存在！", show_alert=True)
...
def genqr_command(call: types.CallbackQuery):
    ...
    with GetDB() as db:
        ...
        if qr_select == 'configs':
            ...
            expiry_date = datetime.fromtimestamp(user.expire).date() if user.expire else "永不"
...
➖➖➖➖➖➖➖➖➖
<u><b>修改前</b></u>
<b>├流量限额：</b> <code>{readable_size(user.data_limit) if user.data_limit else "无限制"}</code>
<b>├到期日期：</b> <code>\
{datetime.fromtimestamp(user.expire).strftime('%H:%M:%S %Y-%m-%d') if user.expire else "永不"}</code>
➖➖➖➖➖➖➖➖➖
<u><b>修改后</b></u>
<b>├流量限额：</b> <code>{readable_size(db_user.data_limit) if db_user.data_limit else "无限制"}</code>
<b>├到期日期：</b> <code>\
{datetime.fromtimestamp(db_user.expire).strftime('%H:%M:%S %Y-%m-%d') if db_user.expire else "永不"}</code>
➖➖➖➖➖➖➖➖➖
<b>操作人：</b> <a href="tg://user?id={call.from_user.id}">{call.from_user.full_name}</a>"""
                try:
                    bot.send_message(TELEGRAM_LOGGER_CHANNEL_ID, text, 'HTML')
                except ApiTelegramException:
                    pass
...
def add_user_bulk_number_step(message: types.Message, username: str):
    try:
        ...
        wait_msg = bot.send_message(message.chat.id, '❌ 数量必须是纯数字.')
        schedule_delete_message(message.chat.id, wait_msg.id)
        schedule_delete_message(message.chat.id, message.id)
        return bot.register_next_step_handler(wait_msg, add_user_bulk_number_step, username=username)
...
def add_user_status_step(call: types.CallbackQuery):
    ...
    if user_status not in ['active', 'onhold']:
        return bot.answer_callback_query(call.id, '❌ Invalid status. 请选择「立即激活」或「暂停等待」。')
...
def add_user_expire_step(message: types.Message, username: str, data_limit: int, user_status: str):
    ...
    bot.send_message(
        message.chat.id, 'Select Protocols:\nUsername: {}\nData Limit: {}\nStatus: {}\nExpiry Date: {}'.format(
            ...
            if mem_store.get(f'{message.chat.id}:data_limit') else "无限制", mem_store.get(
            ...
            if mem_store.get(f'{message.chat.id}:expire_date') else '永不'),
        reply_markup=BotKeyboard.select_protocols(
...
"""
        ...
        with GetDB() as db:
            db_user = crud.get_user(db, username)
            if not db_user:
                return bot.answer_callback_query(call.id, text=f"用户不存在！", show_alert=True)
...
➖➖➖➖➖➖➖➖➖
<b>用户名：</b> <code>{user.username}</code>
<b>Last Traffic Limit :</b> <code>{readable_size(last_user.data_limit) if last_user.data_limit else "无限制"}</code>
<b>New Traffic Limit :</b> <code>{readable_size(user.data_limit) if user.data_limit else "无限制"}</code>{tag}"""
                try:
                    bot.send_message(TELEGRAM_LOGGER_CHANNEL_ID, text, 'HTML')
                except ApiTelegramException:
                    pass
...
{datetime.fromtimestamp(last_user.expire).strftime('%H:%M:%S %Y-%m-%d') if last_user.expire else "永不"}</code>
<b>New Expire Date :</b> <code>\
{datetime.fromtimestamp(user.expire).strftime('%H:%M:%S %Y-%m-%d') if user.expire else "永不"}</code>{tag}"""
                try:
                    bot.send_message(TELEGRAM_LOGGER_CHANNEL_ID, text, 'HTML')
                except ApiTelegramException:
                    pass
        ...
        for i in range(number):
            ...
            for proxy_type in new_user.proxies:
                if not xray.config.inbounds_by_protocol.get(proxy_type):
                    return bot.answer_callback_query(
                        call.id,
                        f'❌ Protocol {proxy_type} 在服务器上未启用',
                        show_alert=True
                    )
            try:
                with GetDB() as db:
...
➖➖➖➖➖➖➖➖➖
<b>用户名：</b> <code>{user.username}</code>
<b>状态：</b> <code>{'活跃' if user_status == 'active' else '暂停中'}</code>
<b>流量限额：</b> <code>{readable_size(user.data_limit) if user.data_limit else "无限制"}</code>
"""
                if user_status == 'onhold':
                    text += f"""\
<b>暂停时长：</b> <code>{new_user.on_hold_expire_duration // (24*60*60)} days</code>
<b>自动启用时间：</b> <code>{new_user.on_hold_timeout.strftime("%H:%M:%S %Y-%m-%d") if new_user.on_hold_timeout else "-"}</code>"""
                else:
                    text += f"""<b>到期日期：</b> \
<code>{datetime.fromtimestamp(user.expire).strftime("%H:%M:%S %Y-%m-%d") if user.expire else "永不"}</code>\n"""
                text += f"""
<b>协议：</b> <code>{"" if not proxies else ", ".join([proxy.type for proxy in proxies])}</code>
➖➖➖➖➖➖➖➖➖
<b>操作人：</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
                try:
                    bot.send_message(TELEGRAM_LOGGER_CHANNEL_ID, text, 'HTML')
...
\t{user.status}\n')
                        ...
                        db.rollback()
            bot.edit_message_text(
                f'✅ <code>{deleted}</code>/<code>{len(depleted_users)}</code> <b>{data[7:].title()} 个用户已删除</b>',
                call.message.chat.id,
                call.message.message_id,
                parse_mode="HTML",
                reply_markup=BotKeyboard.main_menu())
...
<b>操作人：</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
        ...
        with GetDB() as db:
            ...
            bot.edit_message_text(
                f'✅ <b>{data[8:].title()}</b> <code>{inbound}</code> <b>个用户操作成功</b>' +
...
<b>操作人：</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
        ...
        with GetDB() as db:
            ...
            user = UserResponse.model_validate(db_user)
            bot.answer_callback_query(call.id, "✅ 订阅链接已重置！")
            bot.edit_message_text(
                get_user_info_text(db_user),
                call.message.chat.id,
                call.message.message_id,
